...
import random
import string
from wpmock import WackoPickoXssReflected
...
def test(individual, individuals):
    ...
    mock = WackoPickoXssReflected()

    mock.reset()
    for action in individual:
        mock.perform(action)

    individuals, best = FitnessEvaluator().evaluate(mock, individuals)
...
def main(individuals):
    ...
    toolbox.register('population', initPopulation, list, toolbox.individual, length=3, number=20)
